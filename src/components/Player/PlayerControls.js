...
const PlayerControls = () => { 
    ...
    return (
        <div className="player-buttons">
            <button className={`control-btn control-btn-optional ${isShuffle ? 'active' : ''}`} onClick={toggleShuffle} aria-label={t('shuffle')}><Shuffle size={20} /></button>
            ...
            <button className={`control-btn control-btn-optional ${repeatMode !== 'off' ? 'active' : ''}`} onClick={toggleRepeat} aria-label={t('repeat')}><RepeatIcon size={20} /></button>
        </div>
    );
};
